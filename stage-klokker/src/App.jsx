...
export default function App() {
  ...
  // Login form — uses email format: username@stagetimer.app
  ...
  // Firebase auth state listener — replaces session logic
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        if (ud) {
          ...
          // New user with no Firestore doc yet (edge case)
          setCurrentUser(user);
          const newDoc = { username: user.email.split("@")[0], startDate: "", entries: [], clockedIn: null };
  ...
  // Fake email so Firebase Auth is happy (users just pick a username)
  const toEmail = (username) => `${username.toLowerCase().replace(/\s/g, "_")}@stagetimer.app`;

  // ── Auth ──
  const handleLogin = async () => {
    setLoginErr("");
    try {
      await signInWithEmailAndPassword(auth, toEmail(loginUser), loginPass);
      setLoginUser(""); setLoginPass("");
    } catch (e) {
      if (e.code === "auth/user-not-found" || e.code === "auth/invalid-credential") {
        setLoginErr("Gebruiker niet gevonden of wachtwoord onjuist");
      } else {
        setLoginErr("Inloggen mislukt: " + e.message);
      }
    }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const newDoc = { username: loginUser, startDate: "", entries: [], clockedIn: null };
      ...
      if (e.code === "auth/email-already-in-use") {
        setLoginErr("Gebruikersnaam al in gebruik");
      } else {
        setLoginErr("Registreren mislukt: " + e.message);
      }
    }
  };
  ...
  const handleClockIn = async () => {
    ...
    await updateUserData({ clockedIn: time });
    setClockedIn(time);
    showToast("Ingeklokt! 🟢");
  };
  ...
  const handleClockOut = async () => {
    ...
    await updateUserData({ entries: updatedEntries, clockedIn: null });
    setClockedIn(null);
  ...
  const handleManualAdd = async () => {
    ...
    const updatedEntries = [...(userData.entries || []), newEntry];
    await updateUserData({ entries: updatedEntries });
    showToast("Uren handmatig toegevoegd ✅");
    setManStart("09:00"); setManEnd("17:00");
  };

  const handleDeleteEntry = async (id) => {
    const updatedEntries = userData.entries.filter((e) => e.id !== id);
    await updateUserData({ entries: updatedEntries });
  ...
  const handleSaveSettings = async () => {
    await updateUserData({ startDate: settingsStart });
    showToast("Instellingen opgeslagen ✅");
  };

  const handleEndStage = async () => {
    exportExcel(userData.entries || [], userData.startDate, userData.username || currentUser.email);
  ...
  if (loading) {
    return (
      <div style={{
        minHeight: "100vh", background: "#0f0e0d", display: "flex",
        alignItems: "center", justifyContent: "center",
        fontFamily: "'Space Mono', monospace", color: "#ff6b2b", fontSize: 24,
      }}>
        ⏱️
      </div>
    );
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#0f0e0d", color: "#f0ece4",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-btn { flex: 1; padding: 12px 0; background: none; color: #888; font-family: 'DM Sans', sans-serif; font-size: 11px; font-weight: 500; display: flex; flex-direction: column; align-items: center; gap: 4px; transition: color .2s; letter-spacing: .04em; text-transform: uppercase; }
        .nav-btn.active { color: #ff6b2b; }
        .nav-btn svg { width: 22px; height: 22px; }
        .input-field { width: 100%; padding: 14px 16px; background: #1c1a18; border: 1.5px solid #2e2b27; border-radius: 12px; color: #f0ece4; font-family: 'DM Sans', sans-serif; font-size: 16px; transition: border-color .2s; }
        .input-field:focus { border-color: #ff6b2b; }
        .input-field::placeholder { color: #555; }
        .btn-primary { width: 100%; padding: 16px; background: #ff6b2b; color: #fff; border-radius: 12px; font-family: 'DM Sans', sans-serif; font-size: 16px; font-weight: 700; letter-spacing: .02em; transition: background .2s, transform .1s; }
        .btn-primary:active { transform: scale(.97); background: #e55c1e; }
        .btn-secondary { width: 100%; padding: 14px; background: #1c1a18; color: #f0ece4; border-radius: 12px; border: 1.5px solid #2e2b27; font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 500; transition: border-color .2s, transform .1s; }
        .btn-secondary:active { transform: scale(.97); }
        .btn-secondary:hover { border-color: #ff6b2b; color: #ff6b2b; }
        .card { background: #1a1917; border-radius: 16px; border: 1px solid #272421; padding: 20px; }
        .tag { display: inline-block; padding: 3px 10px; border-radius: 6px; font-size: 11px; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; }
        ...
        .clock-btn { width: 200px; height: 200px; border-radius: 50%; border: none; cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 20px; letter-spacing: .06em; text-transform: uppercase; transition: all .25s; box-shadow: 0 0 0 8px rgba(255,107,43,.1), 0 0 0 16px rgba(255,107,43,.04); position: relative; overflow: hidden; }
        .clock-btn::before { content: ''; position: absolute; inset: 0; background: radial-gradient(circle at 30% 30%, rgba(255,255,255,.12), transparent 60%); }
      ...
      {toast && (
        <div className="toast" style={{ position: "fixed", bottom: 90, left: "50%", transform: "translateX(-50%)", background: "#272421", border: "1px solid #3a3530", padding: "12px 22px", borderRadius: 12, fontSize: 14, fontWeight: 500, zIndex: 999, whiteSpace: "nowrap", boxShadow: "0 8px 32px rgba(0,0,0,.5)", color: "#f0ece4" }}>{toast}</div>
      )}
      ...
      {(screen === "login" || screen === "register") && (
        <div className="fade-in" style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "40px 28px" }}>
          <div style={{ marginBottom: 48 }}>
            ...
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 28, fontWeight: 700, color: "#ff6b2b" }}>StageTimer</div>
          ...
          <div style={{ display: "flex", gap: 0, marginBottom: 28, background: "#1a1917", borderRadius: 12, padding: 4 }}>
            {["login", "register"].map((s) => (
              <button key={s} onClick={() => { setScreen(s); setLoginErr(""); }}
                style={{ flex: 1, padding: "10px 0", borderRadius: 9, background: screen === s ? "#ff6b2b" : "none", color: screen === s ? "#fff" : "#888", fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, border: "none", cursor: "pointer", transition: "all .2s" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <input className="input-field" placeholder="Gebruikersnaam" value={loginUser} onChange={(e) => setLoginUser(e.target.value)} autoComplete="username" />
            <input className="input-field" type="password" placeholder="Wachtwoord (min. 6 tekens)" value={loginPass} onChange={(e) => setLoginPass(e.target.value)} autoComplete="current-password"
            ...
            <button className="btn-primary" style={{ marginTop: 8 }} onClick={screen === "login" ? handleLogin : handleRegister}>
      ...
      {!["login", "register"].includes(screen) && (
        <>
              ...
              <div className="fade-in" style={{ padding: "32px 24px 0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32 }}>
                  <div>
                    ...
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 22, fontWeight: 700, color: "#ff6b2b" }}>{displayName}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 26, fontWeight: 700 }}>{pad(now.getHours())}:{pad(now.getMinutes())}</div>
                    <div style={{ color: "#555", fontSize: 12 }}>{fmtDate(now.toISOString())}</div>
                  </div>
                </div>

                <div style={{ display: "flex", justifyContent: "center", marginBottom: 36 }}>
                  <button className={`clock-btn ${clockedIn ? "out" : "in"}`} onClick={clockedIn ? handleClockOut : handleClockIn}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                  <div className="card">
                    ...
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 20, fontWeight: 700, color: "#ff6b2b" }}>{fmtHours(totalHours)}</div>
                  </div>
                  <div className="card">
                    <div style={{ color: "#555", fontSize: 11, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 8 }}>Dagen</div>
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 20, fontWeight: 700 }}>{new Set(entries.map((e) => e.date)).size}</div>
                  </div>
                </div>
                ...
                {userData?.startDate && (
                  <div className="card" style={{ marginBottom: 8 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ fontSize: 14 }}>Week <span style={{ color: "#ff6b2b", fontWeight: 700, fontFamily: "'Space Mono', monospace" }}>{getWeekNumber(now.toISOString().slice(0, 10), userData.startDate)}</span></div>
            ...
            {screen === "hours" && (
              <div className="fade-in" style={{ padding: "32px 24px 0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                  ...
                  <div style={{ background: "#1e1511", border: "1px solid #3a2a1e", borderRadius: 8, padding: "6px 12px", fontFamily: "'Space Mono', monospace", fontSize: 13, color: "#ff6b2b" }}>{fmtHours(totalHours)} totaal</div>
                </div>
                        ...
                        {wkEntries.map((e) => (
                          <div key={e.id} className="card" style={{ marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ color: "#555", fontSize: 12 }}>{fmtDate(e.date)} · {fmtTime(e.clockIn)} – {fmtTime(e.clockOut)}</div>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                              <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 15, fontWeight: 700, color: "#ff6b2b" }}>{fmtHours(e.hours)}</div>
                              <button onClick={() => handleDeleteEntry(e.id)} style={{ background: "none", color: "#555", fontSize: 18, padding: 4, transition: "color .15s" }}
                                onMouseEnter={(el) => el.target.style.color = "#ff4a4a"} onMouseLeave={(el) => el.target.style.color = "#555"}>✕</button>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <div className="fade-in" style={{ padding: "32px 24px 0" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div>
                    ...
                    <input type="date" className="input-field" value={manDate} onChange={(e) => setManDate(e.target.value)} style={{ colorScheme: "dark" }} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      ...
                      <input type="time" className="input-field" value={manStart} onChange={(e) => setManStart(e.target.value)} style={{ colorScheme: "dark" }} />
                    </div>
                    <div>
                      <label style={{ display: "block", color: "#888", fontSize: 12, letterSpacing: ".06em", textTransform: "uppercase", marginBottom: 8 }}>Einde</label>
                      <input type="time" className="input-field" value={manEnd} onChange={(e) => setManEnd(e.target.value)} style={{ colorScheme: "dark" }} />
                    </div>
                  </div>
                  {manStart && manEnd && manDate && (
                    <div className="card" style={{ borderColor: "#2e2b27" }}>
                      ...
                      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 24, fontWeight: 700, color: "#ff6b2b" }}>
                        {(() => { const ms = new Date(`${manDate}T${manEnd}`) - new Date(`${manDate}T${manStart}`); return ms > 0 ? fmtHours(hoursFromMs(ms)) : "ongeldige tijd"; })()}
                      </div>
                    </div>
                  )}
                  <button className="btn-primary" style={{ marginTop: 8 }} onClick={handleManualAdd}>Uren toevoegen →</button>
                </div>
              </div>
            )}
              ...
              <div className="fade-in" style={{ padding: "32px 24px 0" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  <div className="card">
                    ...
                    <input type="date" className="input-field" value={settingsStart} onChange={(e) => setSettingsStart(e.target.value)} style={{ marginBottom: 14, colorScheme: "dark" }} />
                  ...
                  <div className="card">
                    ...
                    <div style={{ color: "#555", fontSize: 13, marginBottom: 14 }}>Download een overzicht van al je uren als Excel bestand.</div>
                  ...
                  <div className="card" style={{ borderColor: "#2a1a1a" }}>
                    ...
                    <div style={{ color: "#555", fontSize: 13, marginBottom: 14 }}>Sluit je stage af. Je totaaloverzicht wordt automatisch als Excel gedownload.</div>
                    {!showEndConfirm ? (
                      <button className="btn-secondary" style={{ borderColor: "#3a1a1a", color: "#ff4a4a" }} onClick={() => setShowEndConfirm(true)}>🎓 Klaar met stage</button>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        <div style={{ color: "#ff6b2b", fontSize: 14, textAlign: "center", padding: "8px 0" }}>Weet je het zeker?</div>
                        <button className="btn-primary" style={{ background: "#c0392b" }} onClick={handleEndStage}>✓ Ja, stage afronden + Excel downloaden</button>
                        <button className="btn-secondary" onClick={() => setShowEndConfirm(false)}>Annuleren</button>
                      </div>
                    )}
                  </div>

                  <button className="btn-secondary" onClick={handleLogout} style={{ color: "#555" }}>Uitloggen</button>
                </div>
              </div>
            )}
          </div>
          ...
          {/* ─── Bottom Nav ─── */}
          <nav style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "#131210", borderTop: "1px solid #272421", display: "flex", paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
            {[
